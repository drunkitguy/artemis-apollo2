...
//    public String customBitrate;
    ...
    // True when the resolution preference is the Full Screen sentinel, meaning
    // width/height above were resolved from a display rather than stored. Game
    // re-resolves them against the display the stream actually lands on.
    public boolean fullscreenResolution;

    // True when the stored bitrate is one we derived rather than one the user
    // chose. Only then may it be recomputed for a different resolved mode.
    public boolean bitrateIsAutoDerived;
    ...
    /**
     * Resolution sentinel meaning "whatever the display in use is, at stream
     * start".
     *
     * <p>This is deliberately not a WxH string. The existing "Native" entries in
     * the resolution list are generated by reading the display when the settings
     * screen is open and then <b>storing the concrete pixels</b>, so a value
     * chosen on the handheld's own panel stays 1080x1920 after docking a 4K
     * display, and vice versa. That is the defect this option exists to remove:
     * the decision has to be deferred to stream start, not baked at pick time.
     *
     * <p>Nothing may rewrite this value. See the guard in {@code readPreferences}.
     */
    public static final String RES_FULLSCREEN = "fullscreen";

    /**
     * Private marker recording whether the stored bitrate is one we derived or
     * one the user chose.
     *
     * <p>Needed because {@link #RES_FULLSCREEN} resolves to a different mode on
     * different displays, so its correct bitrate is not knowable until stream
     * start. Overriding a bitrate the user deliberately set would be worse than
     * the bug being fixed, so the two cases have to be distinguishable.
     *
     * <p>Additive and private: existing installs simply do not have the key,
     * which reads as "not auto" and leaves their bitrate exactly alone. It only
     * becomes true when the user picks Full Screen, which is a new deliberate
     * action.
     */
    static final String BITRATE_IS_AUTO_PREF_STRING = "bitrate_is_auto_derived";

    public static boolean isFullscreenResolution(String resString) {
        return RES_FULLSCREEN.equals(resString);
    }
    ...
    /**
     * Physical pixel size of a display, largest dimension first, or null if it
     * cannot be read.
     *
     * <p>Uses the physical mode rather than the current window size so the
     * answer does not depend on insets, split screen or the caller's window.
     */
    public static android.graphics.Point getDisplayNativeSize(Display display) {
        if (display == null) {
            return null;
        }
        ...
        // Streams are described landscape-first everywhere else in this class,
        // so normalise here rather than leaving every caller to guess.
        return new android.graphics.Point(Math.max(w, h), Math.min(w, h));
    }

    /**
     * The display a context is showing on, falling back to the default display.
     *
     * <p>Deliberately asks the context rather than always taking display 0: on
     * a dual-screen handheld the settings screen and the stream can be on
     * different panels, and the whole point of Full Screen is to follow the
     * display actually in use.
     */
    public static Display getDisplayForContext(Context context) {
        if (context == null) {
            return null;
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            try {
                ...
                // Thrown for non-visual contexts such as the application
                // context. Fall through to the display manager.
            }
        }
    ...
    /** Refresh rate of a display rounded to whole Hz, or 0 if unreadable. */
    public static int getDisplayRefreshRate(Display display) {
        if (display == null) {
            return 0;
        }
        float hz = Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && display.getMode() != null
                ? display.getMode().getRefreshRate()
                : display.getRefreshRate();
        return hz < 1f ? 0 : Math.round(hz);
    }
    ...
    /**
     * Default bitrate for a resolution string, which may be the Full Screen
     * sentinel.
     *
     * <p>{@code context} is only consulted for the sentinel, to resolve it
     * against a real display. It may be null, in which case the sentinel falls
     * back to the default resolution rather than throwing.
     */
    public static int getDefaultBitrate(Context context, String resString, String fpsString) {
        if (isFullscreenResolution(resString)) {
            android.graphics.Point size = getDisplayNativeSize(getDisplayForContext(context));
            if (size == null) {
                resString = DEFAULT_RESOLUTION;
            }
            else {
                return getDefaultBitrate(size.x, size.y, Math.round(Float.parseFloat(fpsString)));
            }
        }
        return getDefaultBitrate(resString, fpsString);
    }

    public static int getDefaultBitrate(String resString, String fpsString) {
        // Callers that might hold the Full Screen sentinel must use the
        // context-taking overload; this one cannot resolve it and parsing it
        // here would throw.
    ...
    /**
     * Default bitrate for an already-resolved mode.
     *
     * <p>This overload exists so the bitrate can be derived from the mode a
     * session actually negotiated rather than the one stored in preferences.
     * Those were the same thing until external-display mode could change the
     * resolution out from under the preference; when they diverged, a 4K60
     * session was requesting the 4K120 figure, 41% over, permanently and with
     * nothing reporting it.
     */
    public static int getDefaultBitrate(int width, int height, int fps) {
    ...
    public static int getDefaultBitrate(Context context) {
        ...
        return getDefaultBitrate(context,
...
//    }
    ...
    public static PreferenceConfiguration readPreferences(Context context, SharedPreferences prefs) {
        ...
        if (str != null) {
            ...
            //
            // The RES_FULLSCREEN guard is load-bearing, not defensive. The
            // sentinel contains no "x", so without it this branch would treat
            // the user's Full Screen choice as a legacy string, convert it to a
            // fixed resolution and WRITE THAT BACK -- silently destroying the
            // setting on the first read after it was chosen.
            if (!PreferenceConfiguration.isFullscreenResolution(resStr) && !resStr.contains("x")) {
            ...
            if (PreferenceConfiguration.isFullscreenResolution(resStr)) {
                config.fullscreenResolution = true;

                // Best effort now, from whatever display this context is on, so
                // the settings screen can show a real number rather than a
                // placeholder. Game re-resolves against the display the stream
                // actually lands on, which is the value that counts.
                android.graphics.Point native_ = getDisplayNativeSize(getDisplayForContext(context));
                if (native_ != null) {
                    config.width = native_.x;
                    config.height = native_.y;
                }
                else {
                    config.width = PreferenceConfiguration.getWidthFromResolutionString(DEFAULT_RESOLUTION);
                    config.height = PreferenceConfiguration.getHeightFromResolutionString(DEFAULT_RESOLUTION);
                }
            }
            else {
        ...
        config.bitrateIsAutoDerived = prefs.getBoolean(BITRATE_IS_AUTO_PREF_STRING, false);
