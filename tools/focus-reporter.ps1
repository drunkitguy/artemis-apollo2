...
param(
    ...
    # Give up if the client cannot be reached for this long, so a crashed
    # Vibepollo that never runs the stop command does not leave this running.
    [int] $QuitAfterIdleSeconds = 300
)
...
try {
    while ($true) {
        ...
        if ($due) {
            ...
            if ($state -ne $last) { $lastChange = $now }
            $last = $state
        }

        if (($now - $lastChange).TotalSeconds -ge $QuitAfterIdleSeconds) {
            break
        }
